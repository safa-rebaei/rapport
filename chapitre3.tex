...
Ce chapitre est consacré à la conception de notre application SalesInsight. 
...
Nous avons utilisé le patron de conception \textbf{MVC (Modèle-Vue-Contrôleur)}, 
un modèle couramment employé pour structurer les applications web en séparant 
les données, la logique métier et l'affichage. Cette séparation des responsabilités 
permet une meilleure répartition du travail au sein de l'équipe et facilite la 
maintenance du code sur le long terme \cite{mdn_mvc}.

\subsection{Composants Clés}

\subsection{Modèle (Model)}
Les modèles définissent la structure des entités métier de notre systéme
et leur persistance en base de données via SQLAlchemy sur SQLite. Chaque entité 
du domaine — client, produit, vente, facturation, notification, utilisateur et 
paramètres société — y est décrite avec ses attributs, ses contraintes et ses 
relations. Cette couche ne contient aucune logique applicative \cite{fastapi_sqlalchemy}.

\subsection{Schéma (Schema)}
Les schémas Pydantic définissent et valident la structure des données échangées 
entre le client et le serveur. Chaque champ y est typé et contraint : valeurs 
minimales, listes non vides, formats attendus. Toute donnée non conforme est 
automatiquement rejetée par FastAPI avec une erreur HTTP 422\cite{pydantic_doc}.

\subsection{Service}
Les services concentrent l'intégralité de la logique métier
 C'est ici que résident les règles spécifiques au domaine : 
vérification de stock avant une vente, décrémentation après validation, génération 
de numéros de vente et de références de facturation, calcul du reste à payer, 
synchronisation du statut actif ou inactif des clients, déclenchement des alertes 
de stock, ainsi que les analyses statistiques et les prévisions via Prophet et 
scikit-learn. Les services peuvent collaborer entre eux de manière descendante, 
sans jamais créer de dépendances circulaires 
\cite{fastapi_services}.

\subsection{Contrôleur (Controller)}
Les contrôleurs, implémentés via les routeurs FastAPI, reçoivent les requêtes 
HTTP, s'appuient sur les schémas pour la validation et délèguent entièrement le 
traitement au service concerné. Cette couche est volontairement minimaliste : 
aucune règle métier n'y figure 
\cite{fastapi_router, fastapi_mvc}.

\subsection{Vue (View)}
Dans le contexte d'une API REST, la vue correspond à la réponse JSON retournée 
au client. Elle est produite par la sérialisation des entités dans les services. 
C'est le frontend React qui assure ensuite l'affichage de ces données à 
l'utilisateur final 
\cite{fastapi_mvc}.

\subsection{Couche Transversale (Cross-Cutting Concerns)}
Plusieurs mécanismes s'appliquent de manière transversale à l'ensemble des 
couches : authentification JWT avec liste noire de tokens, hachage des mots de 
passe via BCrypt, contrôle d'accès par rôle (CEO / agent), et un scheduler 
d'arrière-plan qui génère périodiquement les notifications d'échéance et d'alerte 
de stock 
\cite{jwt_auth, fastapi_background}.

\subsection{Routes}
Les routes organisent les points d'entrée de l'API par domaine fonctionnel et 
appliquent le middleware d'authentification de manière uniforme. Elles assurent 
une communication structurée et sécurisée entre le frontend et le backend 
\cite{fastapi_router}.

\subsection{Architecture Frontend React}
Le frontend applique le même principe de séparation des responsabilités, décliné 
en trois niveaux distincts \cite{react_hooks}.

\subsection{Vue (View)}
Les composants React dans \texttt{src/views/} et \texttt{src/components/} 
assurent l'affichage pur. Ils consomment les données exposées par les hooks sans 
contenir aucune logique applicative ni appel réseau 
\cite{react_hooks}.

\subsection{Contrôleur (Hook)}
Les hooks personnalisés dans \texttt{src/hooks/} gèrent l'état de l'interface, 
orchestrent les actions utilisateur et font le lien entre la vue et la couche API 
\cite{react_hooks}.

\subsection{Couche API}
Les fichiers dans \texttt{src/api/} encapsulent tous les appels HTTP vers le 
backend via Axios. Cette couche est indépendante des composants visuels et ne 
contient aucun état React 
\cite{axios_doc}.

\subsection{Avantages de l'Architecture Adoptée}

\begin{itemize}
    \item \textbf{Séparation des responsabilités.} Chaque couche a un rôle unique et bien délimité, ce qui réduit le couplage entre les modules et simplifie la maintenance à long terme.
    \item \textbf{Testabilité.} Les services étant indépendants de la couche HTTP, la logique métier peut être testée unitairement de manière isolée.
    \item \textbf{Sécurité uniforme.} L'authentification et le contrôle d'accès par rôle sont appliqués au niveau des routes, garantissant une protection cohérente sur l'ensemble des points d'entrée de l'API.
\end{itemize}
\newpage
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/seqConnecter.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.1\textwidth, height=0.25\textheight]{images/mcumcdeconnecter.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/seqDeconnecter.png}%
  }
...
\begin{itemize}[leftmargin=2em, topsep=4pt, itemsep=4pt]
  ...
  \item \textbf{Utilisateur :} l'entité du système qui contient les données relative aux agents
\end{itemize}
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC1_DiagrammeSéquenceAjouterUtilisateur.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=0.95\textwidth, height=0.78\textheight]{images/MC1_DiagrammeClasseGérerUtilisateurs.png}%
  }
...
\begin{itemize}[leftmargin=2em, topsep=4pt, itemsep=4pt]
  ...
  \item \textbf{LigneDeVente :} la classe du système qui contient les données  traitements métier relatives aux produits liés à une vente
\end{itemize}
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC1_DiagrammeSéquenceAjouterProduit_1.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC1_DiagrammeSéquenceModifierProduit.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC1_DiagrammeSéquenceArchiverProduit.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeClasseGérerProduit_2.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_diagseqAjouterClient.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceModifierClient.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceSupprimerClient_1.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_diagrammedeseqArchiverClient.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeClasseGererClient_2.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceCreerFactureviafacture_1.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceConsulterFacture_2.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceEnregistrerRèglement_4.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceArchiverFacture_1.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquenceImprimerFacture.png}%
  }
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.15\textwidth, height=0.78\textheight]{images/MC_DiagrammeSéquence2.png}%
  }
