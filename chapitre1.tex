...
\begin{figure}[H]
    ...
    \caption{Organigramme de Teknovat Solutions}
...
innovante, tout en maintenant un haut niveau d'exigence technique. Durant notre projet de fin d'etude  
nous avons pu  observer comment ces rôles et outils s'articulent pour transformer des idées en solutions 
...
\begin{itemize}[leftmargin=*, noitemsep]
    ...
    et consulter les clients, produits ventes et factures
...
\begin{itemize}[leftmargin=*, noitemsep]
    ...
    Réalisation du modèle de conception en définissant l'architecture logique du système,
    en termes de classes de conception et d'interactions entre objets, en tenant
    compte des contraintes techniques afin de préparer l'implémentation.Réaliser de modéle de deploiment  par l'architecture materielle 
...
\begin{itemize}[leftmargin=*, noitemsep]
 ...
 \cite{audibert_classes}.
    ...
    Représente la séquence de messages entre les objets au cours de la réalisation des séquences d'un cas d'utilisation
 \cite{ibm_sequence}.
...
\subsection{La méthode GIMSI pour les Tableaux De Bord}
...
10 étapes successives, notre projet en mobilise 7, réparties sur 3 phases. Elle
s'inscrit dans un mode de management moderne, fondé sur de solides principes de
gouvernance et de développement durable. La méthode privilégie la coopération
et le partage des connaissances \cite{gimsi}.

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\textwidth}{|>{\centering\bfseries\arraybackslash}p{2.8cm}|>{\centering\arraybackslash}p{0.8cm}|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}X|}
\hline
\textbf{Phase} & \textbf{N°} & \textbf{Étape} & \textbf{Objectifs} \\
\hline
\multirow{2}{*}{\textbf{Identification}}
...
\hline
\end{tabularx}
\caption{Les phases et étapes de la méthode GIMSI appliquées au projet \cite{gimsi_phases}}
\end{table}
...
performante. Les objectifs du projet ont été définis, visant le développement d'un
systéme  de gestion et de suivi des ventes intégrant des fonctionnalités de Business
